...
public class EmployeeInfoServiceImpl implements EmployeeInfoService {
    ...
    public EmployeeInfoServiceImpl(EmployeeInfoRepository employeeInfoRepository, EmployeeInfoMapper employeeInfoMapper) {
    ...
    public Map<String,Object> queryAll(EmployeeInfoQueryCriteria criteria, Pageable pageable){
        Page<EmployeeInfo> page = employeeInfoRepository.findAll((root, criteriaQuery, criteriaBuilder) -> QueryHelp.getPredicate(root,criteria,criteriaBuilder),pageable);
    ...
    public List<EmployeeInfoDto> queryAll(EmployeeInfoQueryCriteria criteria){
        return employeeInfoMapper.toDto(employeeInfoRepository.findAll((root, criteriaQuery, criteriaBuilder) -> QueryHelp.getPredicate(root,criteria,criteriaBuilder)));
    }
    ...
    public void download(List<EmployeeInfoDto> all, HttpServletResponse response) throws IOException {
        ...
        for (EmployeeInfoDto employeeInfo : all) {
            ...
            map.put("住址", employeeInfo.getAddress());
            list.add(map);
        }
